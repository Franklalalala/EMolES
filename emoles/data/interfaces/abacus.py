...
import h5py
from emoles.utils.constants import orbitalId, Bohr2Ang, ABACUS2EMolES
import ase
import pickle
import lmdb


class OrbAbacus2EMolES:
    def __init__(self):
        # self.Us_abacus2EMolES = {}
        # self.Us_abacus2EMolES[0] = np.eye(1)
        # self.Us_abacus2EMolES[1] = np.eye(3)[[2, 0, 1]]            # 0, 1, -1 -> -1, 0, 1
        # self.Us_abacus2EMolES[2] = np.eye(5)[[4, 2, 0, 1, 3]]      # 0, 1, -1, 2, -2 -> -2, -1, 0, 1, 2
        # self.Us_abacus2EMolES[3] = np.eye(7)[[6, 4, 2, 0, 1, 3, 5]] # -3,-2,-1,0,1,2,3
        # self.Us_abacus2EMolES[4] = np.eye(9)[[8, 6, 4, 2, 0, 1, 3, 5, 7]]
        # self.Us_abacus2EMolES[5] = np.eye(11)[[10, 8, 6, 4, 2, 0, 1, 3, 5, 7, 9]]

        self.Us_abacus2EMolES = ABACUS2EMolES
        ...
        #     self.Us_abacus2EMolES[k][v] *= -1  # add phase (-1)^m

    def get_U(self, l):
        if l > 5:
            raise NotImplementedError("Only support l = s, p, d, f, g, h.")
        return self.Us_abacus2EMolES[l]
