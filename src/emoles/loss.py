...
def calculate_dm_dipole_mae(pred_dm, target_dm, mol):
    ...
    Uses the same dipole calculation method as the original code.
    """
    error_dict = {}

    # Density matrix MAE - same calculation as original
    diff_matrix = np.abs(np.array(pred_dm - target_dm))
    error_dict['density_matrix'] = np.mean(diff_matrix)

    # Dipole moment error - same calculation as original
...
def get_electronic_properties(mol, ham=None, overlap=None, dm=None, shifted_ham=None):
    """
    ...
    energies, coeffs = cal_orbital_and_energies(overlap_matrix=ov_in_3d, full_hamiltonian=ham_in_3d)
...
def evaluate_dm_from_npy(abs_ase_path,
                         npy_folder_path,
                         convention='def2svp',
                         mol_charge=0,
    ...
    with connect(abs_ase_path) as db:
        for idx, a_row in tqdm(enumerate(db.select())):
            ...
            try:
                ...
                mol = pyscf.gto.Mole()
                t = [[atom_nums[atom_idx], an_atom.position] for atom_idx, an_atom in enumerate(an_atoms)]
                ...
                if get_ham_flag:
                    pred_props = get_electronic_properties(mol, dm=pred_dm)
                    target_props = get_electronic_properties(mol, dm=target_dm)

                    eval_keys = ['hamiltonian', 'HOMO', 'LUMO', 'GAP',
                                 'orbital_coefficients', 'HOMO_coefficients', 'LUMO_coefficients']

                    ham_orb_errors = criterion(pred_props, target_props, eval_keys, flag=False, atoms=an_atoms, mol=mol)
                    errors.update(ham_orb_errors)

                    # --- 新增逻辑：保存用于 generate_cube_files 的数据 ---
                    if idx < n_save_cube_items:
                        ...
                        cube_item = {
                            'idx': idx,
                            'HOMO_sim': errors.get('HOMO_coefficients', 0.0),  # 获取相似度
                            'mol_info': mol_info,
                            'outputs': pred_props,  # 包含 orbital_coefficients 等
                            'tgt_info': target_props  # 包含 orbital_coefficients 等
                        }
                ...
                if get_esp_sta_flag:
                    p_esp_max, p_esp_min, p_phi = calculate_properties_from_dm(mol, pred_dm, "pred", gen_dm_flag=False)
                    t_esp_max, t_esp_min, t_phi = calculate_properties_from_dm(mol, target_dm, "target",
                                                                               gen_dm_flag=False)
                ...
                mol_info_log = {'formula': an_atoms.get_chemical_formula(), 'charge': int(current_mol_charge),
                                'spin': int(mol_spin)}
    ...
    if temp_data_file and len(temp_cube_data) > 0:
        # 保存到 npy_folder_path 或者直接保存到当前路径，这里假设保存到 npy_folder_path 下以便管理
        save_path = os.path.join(npy_folder_path, temp_data_file)
        try:
            with open(save_path, "wb") as f:
                pickle.dump(temp_cube_data, f)
            print(f"[evaluate_dm_from_npy] Saved cube info for {len(temp_cube_data)} items to {save_path}")
        except Exception as e:
    ...
    final_data_to_process = {'pred_vs_label': total_error_dict['pred_vs_label'].copy()}
    final_data_to_process['pred_vs_label']['second_per_item'] = total_error_dict['second_per_item']
    ...
    print(f"[evaluate_dm_from_npy] Attempted: {attempted_count}, Success: {n}, Failed: {fail_count}")
    return result_dict
...
def get_mae_from_npy(abs_ase_path,
                     npy_folder_path,
                     temp_data_file=None,
    ...
    with connect(abs_ase_path) as db:
        for idx, a_row in tqdm(enumerate(db.select())):
            ...
            pred_ham = _load_npy_safe(os.path.join(npy_folder_path, f'{idx}', 'predicted_ham.npy'))
            orig_ham = _load_npy_safe(os.path.join(npy_folder_path, f'{idx}', 'original_ham.npy'))
            ...
            if not united_overlap_flag:
                pred_ov = _load_npy_safe(os.path.join(npy_folder_path, f'{idx}', 'predicted_overlap.npy'))
                orig_ov = _load_npy_safe(os.path.join(npy_folder_path, f'{idx}', 'original_overlap.npy'))

                orig_ham_prep, orig_ov_prep = prepare_np(atom_symbols=atom_nums,
                                                         overlap_matrix=orig_ov,
                                                         full_hamiltonian=orig_ham,
                                                         transform_ham_flag=True,
                                                         transform_overlap_flag=True,
                                                         convention=convention)
                pred_ham_prep, pred_ov_prep = prepare_np(atom_symbols=atom_nums,
                ...
                shifted_label_ham = get_shifted_ham(predicted_ham=pred_ham_bt,
            ...
            outputs = get_electronic_properties(mol, ham=pred_ham_prep, overlap=pred_ov_prep)
            tgt_info = get_electronic_properties(mol, ham=orig_ham_prep, overlap=orig_ov_prep,
    ...
    if n > 0:
        for key in total_error_dict['pred_vs_label'].keys():
            total_error_dict['pred_vs_label'][key] = total_error_dict['pred_vs_label'][key] / n
